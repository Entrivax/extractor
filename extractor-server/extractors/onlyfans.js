;(function(websocketUrl, rules) {
    ...
    async function getSign(path, time) {
        ...
        const checksum = checksumMap.reduce((a, b) => a + sha1AsString.charCodeAt(b), rules.checksum_constant)

        return processPyFormat(rules.format, sha1AsString, checksum)
    }

    function processPyFormat(str, ...args) {
        let concat = ''
        let interpoling = false
        let argIndex = 0
        for (let i = 0; i < str.length; i++) {
            if (interpoling) {
                let indexOfCloseBracket = str.indexOf('}', i)
                if (indexOfCloseBracket === -1) {
                    concat += str.slice(i)
                    break
                } else {
                    let interpolationContent = str.slice(i, indexOfCloseBracket)
                    switch (interpolationContent) {
                        case ':x':
                            concat += args[argIndex].toString(16)
                            break
                        default:
                            concat += args[argIndex]
                            break
                    }
                    argIndex++
                    i = indexOfCloseBracket
                    interpoling = false
                }
                continue
            }
            let indexOfOpenBracket = str.indexOf('{', i)
            if (indexOfOpenBracket !== -1) {
                concat += str.slice(i, indexOfOpenBracket)
    ...
    async function extractFromCurrentOnlyfansPage(ws, zipId, creator, loadedBackupId) {
        ...
        const finalData = data.slice(0)

        // Merge pinned posts to the posts array
        if (pinnedPosts) {
            for (let i = 0; i < pinnedPosts.length; i++) {
                if (!finalData.find(p => p.id === pinnedPosts[i].id)) {
                    finalData.push(pinnedPosts[i])
                }
            }
        }

        finalData.sort((p1, p2) => +new Date(p2.postedAt) - +new Date(p1.postedAt))
