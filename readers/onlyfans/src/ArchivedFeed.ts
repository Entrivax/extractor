...
export class ArchivedFeed {
    ...
    set elementsPerPage(value: number) {
        ...
        let newValue = value
        if (newValue === null) {
            this.router.navigateToRoute('archived', { limit: 'all' })
        } else {
            const firstElementNum = (oldValue || 0) * (this.page || 0)
            newValue ||= 50
            const newFirstElementPage = Math.floor(firstElementNum / (newValue))
            this.router.navigateToRoute('archived', { page: (newFirstElementPage + 1).toString(), limit: value.toString() })
        }
    }
    ...
    async attached() {
        ...
        try {
            const posts: any[] = data.archivedPosts
